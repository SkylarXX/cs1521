...
int main (void)
{
	...
	uid_t uid = getuid ();
	in_port_t port = PORTUID_BASE + (uid % PORTUID_MOD) + 29;
	...
	int *n_requests, int *n_responses)
{
	...
	if(strcmp(token, "/hello") == 0) {
	    ...
	    time_t currTime;
	    time(&currTime);
	    responStr = ctime(&currTime);
	    
	    strcat(response, responStr);
	    
	} else if(strcmp(token, "/hello") > 0) {
	    char name[BUFSIZ];
	    
	    sscanf(token, "%*[/hello?]%s", name);
	    printf("%s\n", name);
	    strcat(response, "<h2>");
	...
	} else if(strcmp(token, "/nonexistent") == 0) {
