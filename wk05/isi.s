...
# Frame:	$fp, $ra, $s0, $s1, $s2, $s3
# Uses:	    $a0, $a1, $t0, $t1, $t2, $t3, $s0, $s1, $s2, $s3
...
isIdent_row_init:
    li    $s2, 0    # row = 0
isIdent_row_cond:
	bge   $s2, $s1, isIdent_row_f    # row >= n, goes to end
	
isIdent_col_init:
    li    $s3, 0    # col = 0
isIdent_col_cond:
    bge   $s3, $s1, isIdent_col_f    # col >= n, goes to end
    
    bne   $s2, $s3, isIdent_elif     # row != col, jumo to elif
    
isIdent_if:
    li    $t0, 1
    li    $t1, 4           # sizeof(int)
    mul   $t2, $s1, $t1    # rowsize = #col * sizeof(int)
    mul   $t2, $s2, $t2    # t2 = row * rowsize
    mul   $t3, $s3, $t1    # t3 = col * sizeof(int)
    add   $t3, $t3, $t2    # offset
    
    addu  $t3, $s0, $t3    # (row * N) + col
    
    lw    $t3,($t3)
    
    beq   $t3, $t0, isIdent_col_step    # m[row][col] == 1
    
    li    $v0, 0
    j     isIdent_end              # return 0
isIdent_elif:
    ...
    mul   $t2, $s1, $t1    # rowsize = #col * sizeof(int)
    mul   $t2, $s2, $t2    # t2 = row * rowsize
    mul   $t3, $s3, $t1    # t3 col * sizoef(int)
    add   $t3, $t3, $t2
    addu  $t3, $s0, $t3    # (row * N) + col
    
    lw    $t3,($t3)
    
    beq   $t3, $t0, isIdent_col_step    # m[row][col] == 0
